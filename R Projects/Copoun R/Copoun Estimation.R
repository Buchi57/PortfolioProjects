##############################################################################
### Main result for The proposed new Copoun distribution
####################################################################################################
library(dplyr)
library(maxLik)
...
CAIC
##################################################
# Assuming the previous MLE code has been executed

# Extract estimated parameters
theta_hat <- coef(ml)["theta"]
alpha_hat <- coef(ml)["alpha"]

# Plot histogram with density scaling
hist(datas14, freq = FALSE, breaks = "FD", col = "lightblue",
     main = "Histogram with Fitted Distribution", xlab = "Value")

# Define the fitted PDF function
fitted_pdf <- function(x) {
  (theta_hat^2 / (alpha_hat + theta_hat)) * 
  (1 + (alpha_hat * theta_hat^2 * x^3) / 6) * 
  exp(-theta_hat * x)
}

# Add the fitted PDF curve
curve(fitted_pdf(x), from = 0, to = max(datas14), 
      add = TRUE, col = "red", lwd = 2)

# Optional: Add a legend
legend("topright", legend = "Fitted PDF", col = "red", lwd = 2) 

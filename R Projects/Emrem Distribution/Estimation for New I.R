...
set.seed(0)
fitnew1 <- goodness.fit(pdf = pdf_New1, cdf = cdf_New1,       
 starts = c(0.5,0.5), data = datas1, method = "PSO",         
...
resnew1 = goodness.fit(pdf = pdf_New1, cdf = cdf_New1, 
                      starts = c(0.5,1), data = datas1, method = "PSO",
                      domain = c(0,Inf),mle = NULL, lim_inf = c(0.5,0.5),
                      lim_sup = c(5,10), S = 5550, prop=0.1, N=50)
result_1$mle
x = seq(0, 6, length.out = 500)
hist(datas1, probability = TRUE)
lines(x, pdf_New1(x, par = result_1$mle), col = "blue")
##############################################################################################################################
...
datas2 <- datas2 %>%
  na.omit()
set.seed(0)
resnew2 = goodness.fit(pdf = pdf_New1, cdf = cdf_New1, 
